...
modelName="2D.ConstantSize.Migration"
...
def split_Wmig(params, ns, pts):
    ...
    phi = Integration.two_pops(phi, xx, TDiv, nu1, nu2, m12=m, m21=m)  # two pops at diff sizes with symmetric migration
    # allow for another size change; fix time at 35 gen
    fs = Spectrum.from_phi(phi, ns, (xx,xx))
    return fs
param_names=("nu1","nu2","TDiv","m")
...
m_fraction=popt[3]/(2*Nanc) # migration rates come out scaled by fraction of members of pop  scaled * 2Na according to dadi manual so here you actually DIVIDE by 2Na *not multiply* to get the migration fraction. Tricky tricky :)
# THIS IS TRICKY: confirmed here: https://groups.google.com/forum/#!topic/dadi-user/AY_4NoEjsx8
scaled_param_names=("Nanc_FromTheta_scaled_dip","nu1_scaled_dip","nu2_scaled_dip","T_scaled_gen","migrationFraction")
scaled_popt=(Nanc,nu1_scaled_dip,nu2_scaled_dip,T_scaled_gen,m_fraction)
