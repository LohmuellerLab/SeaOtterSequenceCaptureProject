...
# USAGE: ./make_slim_otter_bottleneck_recovery_add_load_remove_fixed_031120.sh [Na] [Nb] [nF] [Nr] [h]
...
# Make script
cat > slim_otter_bottleneck_recovery_add_load_remove_fixed_${Na}Na_${Nb}Nb_${nF}nF_${Nr}Nr_h${h}_031120.slim << EOM
...
function (s) getStats(o pop, i sampSize)
{
	...
	for (individual in i) {
		...
		// calculate individual fitness - code from Bernard - modified to get load additive across sites
