...
# USAGE: ./make_slim_otter_bottleneck_recovery_add_load_remove_fixed_hmix_073120.sh [Na] [Nb] [nF] [Nr]
...
# Make script
cat > slim_otter_bottleneck_recovery_add_load_remove_fixed_hmix_${Na}Na_${Nb}Nb_${nF}nF_${Nr}Nr_073120.slim << EOM
...
initialize() {
	...
	//cat("Genome length:"+seqLength+"\n");
        ...
        // so need to increase mutation rate (8.64e-9) by (2.31/3.31)/0.5=1.23e-8
        ...
        initializeMutationRate(1.23e-8);
